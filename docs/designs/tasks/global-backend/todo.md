...
- [ ] Task 7: Add the `/uploads` media ingestion API
  - Acceptance: upload route returns `mediaId`; assets are sanitized and stored under `{root}/uploads/`; assets are represented in `config.media[]`; conflicting current media routes are removed or rewritten.
  - Verify: `rtk pnpm -F @vc/server test`, `rtk pnpm -F @vc/server lint`
